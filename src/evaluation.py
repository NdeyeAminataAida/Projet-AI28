...
def plot_confusion_matrix(model, X, y_true, title="Matrice de confusion"):
    ...
    Une matrice de confusion par modèle permet de visualiser directement les erreurs :
    faux positifs (clients sans défaut prédits en défaut) et surtout faux négatifs
    (défauts non détectés), qui sont les plus coûteux pour la banque.
...
def optimize_model(build_fn, suggest_fn, X_train, y_train, X_val, y_val,
                   scoring="f1", cv=5, n_trials=25, random_state=42):
    '''
    Optimise les hyper-paramètres d'UN modèle avec Optuna (échantillonneur TPE),
    puis réentraîne la meilleure pipeline sur tout X_train et l'évalue sur la validation.

    Remplace GridSearchCV pour l'optimisation modèle par modèle : au lieu d'une grille
    exhaustive de valeurs discrètes, Optuna échantillonne des plages continues
    (C / learning_rate en échelle logarithmique, profondeurs et nombres d'arbres en
    entiers) de façon plus fine et plus efficace, via une optimisation bayésienne.

    Parameters
    ----------
    build_fn : callable () -> Pipeline
        Fonction sans argument renvoyant une pipeline NEUVE (Optuna repart d'un
        estimateur vierge à chaque essai).
    suggest_fn : callable(trial) -> dict
        Renvoie les hyper-paramètres tirés par Optuna {clé_pipeline: valeur}, avec
        des clés compatibles set_params (ex. 'decisiontreeclassifier__max_depth'),
        si bien que study.best_params se réinjecte directement.
    scoring : str
        Métrique maximisée en validation croisée. Par défaut 'f1' (adaptée au
        déséquilibre des classes) ; passer 'average_precision' pour optimiser
        directement le PR-AUC.
    cv, n_trials, random_state :
        Nombre de plis de validation croisée, budget d'essais Optuna et graine du
        sampler (pour une optimisation reproductible).

    Returns
    -------
    Pipeline : la meilleure pipeline, réentraînée sur tout X_train.
    '''
    ...
    print(f"Optimisation Optuna en cours ({n_trials} essais)...")
    study.optimize(objective, n_trials=n_trials)

    # Affichage des résultats de la recherche
    print("\n--- Résultats de l'optimisation Optuna ---")
    print("Meilleurs hyper-paramètres trouvés :", study.best_params)
    print(f"Meilleur score {scoring} en validation croisée : {study.best_value:.3f}")

    # Réentraînement de la meilleure pipeline sur tout X_train (Optuna ne le fait pas seul),
    # puis affichage des classification reports sur train et validation.
    best_model = build_fn()
    best_model.set_params(**study.best_params)
    best_model = train_and_evaluate(best_model, X_train, y_train, X_val, y_val)
    return best_model
